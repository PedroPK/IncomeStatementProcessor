...
- `POST /api/shutdown` → `os.kill(os.getpid(), SIGTERM)` após 0,4 s; fallback `os._exit(0)` após 1,5 s adicional
- `window.close()` fecha a aba do navegador após enviar a requisição
